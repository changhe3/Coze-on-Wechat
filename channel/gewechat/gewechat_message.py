...
class GeWeChatMessage(ChatMessage):
    def __init__(self, msg, client: GewechatClient):
        ...
        if msg_type == 1:  # Text message
            ...
            self.content = TmpDir().path() + str(self.msg_id) + ".jpg"
    ...
    def download_image(self):
        ...
        try:
            ...
            # 优先尝试高清图(type=1)，失败后自动降级普通图(type=2)
            for img_type in [1, 2]:
                try:
                    ...
                    if image_info.get('ret') == 200 and image_info.get('data'):
                        return self._process_image_download(image_info)
                except Exception as e:
                    logger.warning(f"[gewechat] {['High-quality', 'Normal'][img_type-1]} image download failed: {str(e)}")
                    if img_type == 2:  # 两种类型都失败时抛出异常
                        raise
            logger.error(f"[gewechat] All image download attempts failed")
        except Exception as e:
            logger.error(f"[gewechat] Image download failed: {str(e)}")
            raise  # 抛出异常给上层处理

    def _process_image_download(self, image_info):
        """处理图片下载通用逻辑"""
        file_url = image_info['data']['fileUrl']
        logger.info(f"[gewechat] Downloading image from {file_url}")

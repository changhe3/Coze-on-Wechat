...
try:
    from voice.audio_convert import sil_to_wav
...
class ChatChannel(Channel):
    ...
    def _handle(self, context: Context):
        ...
        # 修改点：处理多个回复的情况
        replies = self._generate_reply(context)
        
        # 当返回的是单个Reply时转换为列表统一处理
        ...
        for reply in replies:
            ...
            decorated_reply = self._decorate_reply(context, reply)
            if decorated_reply and decorated_reply.content:
                self._send_reply(context, decorated_reply)

    def _generate_reply(self, context: Context, reply: Reply = Reply()) -> list:
